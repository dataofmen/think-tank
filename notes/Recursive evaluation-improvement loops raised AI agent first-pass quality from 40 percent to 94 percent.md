...
The pattern mirrors autoresearch: a measurable metric (pass rate), code-level access (generation layer prompts), and experiment history (failure pattern logs) form the loop. The key constraint is that the task must have efficiently evaluable criteria — structured outputs like lead responses are ideal, while creative or ambiguous tasks may resist clean pass/fail decomposition.
